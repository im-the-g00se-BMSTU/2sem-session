...
order: 18
---
# 25. Динамические структуры данных. Односвязный список, стек и очередь. Алгоритмы их обработки.

Динамические структуры данных<br>Динамические структуры данных - это составные структуры данных, которые могут изменять свой размер во время выполнения программы.
...
К динамическим структурам относятся: односвязный список, стек, очередь

Односвязный список<br>Односвязный линейный список - это динамическая структура данных, состоящая из узлов. Каждый узел хранит:
...
```c
typedef struct Node {// реализация узла односвязного списка
   void* data;
   struct Node* next;
} Node;
```
...
```c
typedef struct {//структура самого списка
   Node* head;
   size_t size;
} List;
```
...
Алгоритм добавления узла<br>Функция добавления узла принимает:
...
Пример:<br>Node\* newNode = malloc(sizeof(Node));<br>newNode-\>data = data;<br>newNode-\>next = prev-\>next;<br>prev-\>next = newNode;

Если вставка идёт в начало списка:<br>newNode-\>next = list-\>head;<br>list-\>head = newNode;
...
![Изображение](assets/image18.png)<br>Очередь - это динамическая структура данных, организованная по принципу: FIFO - First In, First Out. То есть первый добавленный элемент извлекается первым.
...
Queue\* createQueue(size\_t dataSize) {//создание очереди
